"""9n — TDK GTS (12. baskı) `lisan` alanından YENİ rapor altını, DOĞAL oranlarda (9m kuralı + 9m altını dışarıda).
...
9l, 9m altınlarının KELİMELERİ ve (verici, etimon) GRUPLARI yok; grup başına tek madde.
...
ve fazlası diğerlerine dağıtılmaz (oran korunur). Seçim tuzlu hash sırası (`donor9n-tdk-v1`).
Altın dosyasında yalnız kelime, sınıf, kısa etimon. Tek bölüm `rapor`.

python data/cache/work/donor9n/build_gold.py
"""
...
def main():
    ...
    meta = {"salt": SALT, "N": N, "source": "TDK GTS v12 lisan; TDK+Nişanyan, 9e/9f/9g, 9j-TDK, 9j-TETTL, 9l, 9m dışı",
