"""9m — TDK GTS (12. baskı) `lisan` alanından YENİ rapor altını, DOĞAL oranlarda.
...
9l altınlarının KELİMELERİ ve (verici, etimon) GRUPLARI yok; grup başına tek madde.
...
ve fazlası diğerlerine dağıtılmaz (oran korunur). Seçim tuzlu hash sırası (`donor9m-tdk-v1`).
Altın dosyasında yalnız kelime, sınıf, kısa etimon. Tek bölüm `rapor`.

python data/cache/work/donor9m/build_gold.py
"""
...
SALT = "donor9m-tdk-v1"
N = 600
NAT = json.loads((OUT / "natural.json").read_text())["tum"]
...
def main():
    ...
    meta = {"salt": SALT, "N": N, "source": "TDK GTS v12 lisan; TDK+Nişanyan, 9e/9f/9g, 9j-TDK, 9j-TETTL, 9l dışı",
